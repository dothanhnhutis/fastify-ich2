...
-- FindManyWarehouseService v1
SELECT w.*,
       (SELECT COUNT(*)
        FROM packaging_inventory pi2
                 JOIN packagings p2 ON p2.id = pi2.packaging_id AND p2.deleted_at IS NULL
        WHERE pi2.warehouse_id = w.id)::int AS packaging_count,
...
FROM warehouses w
    ...
    FROM packagings p
             ...
             LEFT JOIN packaging_images pim
                       ON pim.packaging_id = p.id AND pim.is_primary = TRUE AND p.deleted_at IS NULL
             LEFT JOIN files f ON f.id = pim.file_id
