...
CREATE TRIGGER trg_updated_at_packagings
    BEFORE
        UPDATE
    ON packagings
    FOR EACH ROW
...
CREATE TRIGGER trg_updated_at_packaging_inventory
    BEFORE
        UPDATE
    ON packaging_inventory
    FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

CREATE TRIGGER trg_updated_at_packaging_transactions
    BEFORE
        UPDATE
    ON packaging_transactions
    FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

CREATE TRIGGER trg_updated_at_packaging_transaction_items
    BEFORE
        UPDATE
    ON packaging_transaction_items
    FOR EACH ROW
EXECUTE FUNCTION set_updated_at();

-- trigger nếu type !==CARTON thì auto set pcs_ctn = NULL
CREATE TRIGGER trg_set_pcs_ctn_null
    BEFORE INSERT
        OR
        UPDATE
    ON packagings
...
CREATE TRIGGER trg_set_to_warehouse_null
    BEFORE INSERT
        OR
        UPDATE
    ON packaging_transactions
